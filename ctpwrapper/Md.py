...
class AsyncMdApiPy(MdApiWrapper):
    def __init__(self):
    ...
    async def Create(self, pszFlowPath: Optional[str] = "",
        ...
        :param bIsMulticast:
        """
        super(AsyncMdApiPy, self).Create(pszFlowPath.encode(), bIsUsingUdp, bIsMulticast)

    async def Init(self) -> None:
        """
        初始化运行环境,只有调用后,接口才开始工作
        """
        super(AsyncMdApiPy, self).Init()
        await asyncio.sleep(1)  # wait for c++ init
    ...
    async def Join(self) -> int:
        ...
        @return 线程退出代码
        """
        return await self.loop.run_in_executor(None, super(AsyncMdApiPy, self).Join)

    async def Release(self) -> None:
        super(AsyncMdApiPy, self).Release()

    async def ReqUserLogin(self, pReqUserLogin: "ReqUserLoginField", nRequestID: int) -> int:
        """
        用户登录请求
        """
        return super(AsyncMdApiPy, self).ReqUserLogin(pReqUserLogin, nRequestID)

    async def ReqUserLogout(self, pUserLogout: "UserLogoutField", nRequestID: int) -> int:
        """
         登出请求
        """
        return super(AsyncMdApiPy, self).ReqUserLogout(pUserLogout, nRequestID)

    async def ReqQryMulticastInstrument(self, pQryMulticastInstrument: "QryMulticastInstrumentField", nRequestID: int) -> int:
        """
        请求查询组播合约
        """
        return super(AsyncMdApiPy, self).ReqQryMulticastInstrument(pQryMulticastInstrument, nRequestID)

    async def GetTradingDay(self) -> str:
        """
        获取当前交易日
        @retrun 获取到的交易日
        @remark 只有登录成功后,才能得到正确的交易日
        :return:
        """
        day = super(AsyncMdApiPy, self).GetTradingDay()
        return day.decode()

    async def RegisterFront(self, pszFrontAddress: str) -> None:
        """
        注册前置机网络地址
        @param pszFrontAddress：前置机网络地址。
        @remark 网络地址的格式为：“protocol:# ipaddress:port”，如：”tcp:# 127.0.0.1:17001”。
        @remark “tcp”代表传输协议，“127.0.0.1”代表服务器地址。”17001”代表服务器端口号。
        """
        super(AsyncMdApiPy, self).RegisterFront(pszFrontAddress.encode())

    async def RegisterNameServer(self, pszNsAddress: str) -> None:
        """
        注册名字服务器网络地址
        @param pszNsAddress：名字服务器网络地址。
        @remark 网络地址的格式为：“protocol:# ipaddress:port”，如：”tcp:# 127.0.0.1:12001”。
        ...
        @remark RegisterNameServer优先于RegisterFront
        """
        super(AsyncMdApiPy, self).RegisterNameServer(pszNsAddress.encode())

    async def RegisterFensUserInfo(self, pFensUserInfo: "FensUserInfoField") -> None:
        """
        注册名字服务器用户信息
        @param pFensUserInfo：用户信息。
        """
        super(AsyncMdApiPy, self).RegisterFensUserInfo(pFensUserInfo)

    async def SubscribeMarketData(self, pInstrumentID: typing.List[str]) -> int:
        """
         订阅行情。
        @param pInstrumentID 合约ID
        :return: int
        """
        ids = [bytes(item, encoding="utf-8") for item in pInstrumentID]
        return super(AsyncMdApiPy, self).SubscribeMarketData(ids)

    async def UnSubscribeMarketData(self, pInstrumentID: typing.List[str]) -> int:
        """
        退订行情。
        @param pInstrumentID 合约ID
        :return: int
        ...
        return super(AsyncMdApiPy, self).UnSubscribeMarketData(ids)

    async def SubscribeForQuoteRsp(self, pInstrumentID: typing.List[str]) -> int:
        """
        订阅询价。
        :param pInstrumentID: 合约ID list
        :return: int
        ...
        return super(AsyncMdApiPy, self).SubscribeForQuoteRsp(ids)

    async def UnSubscribeForQuoteRsp(self, pInstrumentID: typing.List[str]) -> int:
        """
        退订询价。
        :param pInstrumentID: 合约ID list
        :return: int
        ...
        return super(AsyncMdApiPy, self).UnSubscribeForQuoteRsp(ids)

    # for receive message
    async def OnFrontConnected(self) -> None:
        """
        当客户端与交易后台建立起通信连接时（还未登录前），该方法被调用。
        :return:
        """
    ...
    async def OnFrontDisconnected(self, nReason) -> None:
        """
        当客户端与交易后台通信连接断开时，该方法被调用。当发生这个情况后，API会自动重新连接，客户端可不做处理。
        @param nReason 错误原因
            4097 0x1001 网络读失败
    ...
    async def OnHeartBeatWarning(self, nTimeLapse) -> None:
        """
        心跳超时警告。当长时间未收到报文时，该方法被调用。
    ...
    async def OnRspUserLogin(self, pRspUserLogin, pRspInfo, nRequestID, bIsLast) -> None:
        """
        登录请求响应
        :param pRspUserLogin:
        :param pRspInfo:
    ...
    async def OnRspUserLogout(self, pUserLogout, pRspInfo, nRequestID, bIsLast) -> None:
        """
        登出请求响应
        :param pUserLogout:
        :param pRspInfo:
    ...
    async def OnRspQryMulticastInstrument(self, pMulticastInstrument, pRspInfo, nRequestID, bIsLast) -> None:
        """
        请求查询组播合约响应
        :param pMulticastInstrument:
        :param pRspInfo:
    ...
    async def OnRspError(self, pRspInfo, nRequestID, bIsLast) -> None:
        """
        错误应答
        :param pRspInfo:
        :param nRequestID:
    ...
    async def OnRspSubMarketData(self, pSpecificInstrument, pRspInfo, nRequestID, bIsLast) -> None:
        """
        订阅行情应答
        :param pSpecificInstrument:
        :param pRspInfo:
    ...
    async def OnRspUnSubMarketData(self, pSpecificInstrument, pRspInfo, nRequestID, bIsLast) -> None:
        """
        取消订阅行情应答
        :param pSpecificInstrument:
        :param pRspInfo:
    ...
    async def OnRspSubForQuoteRsp(self, pSpecificInstrument, pRspInfo, nRequestID, bIsLast) -> None:
        """
        订阅询价应答
        :param pSpecificInstrument:
        :param pRspInfo:
    ...
    async def OnRspUnSubForQuoteRsp(self, pSpecificInstrument, pRspInfo, nRequestID, bIsLast) -> None:
        """
        取消订阅询价应答
        :param pSpecificInstrument:
        :param pRspInfo:
    ...
    async def OnRtnDepthMarketData(self, pDepthMarketData) -> None:
        """
        深度行情通知
        :param pDepthMarketData:
        :return:
    ...
    async def OnRtnForQuoteRsp(self, pForQuoteRsp) -> None:
        """
        询价通知
        :param pForQuoteRsp:
        :return:
